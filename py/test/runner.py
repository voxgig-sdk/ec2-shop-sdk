...
class Ec2ShopTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            Ec2ShopTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = Ec2ShopTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = Ec2ShopTestRunner.getenv("EC2_SHOP_TEST_LIVE")
        override = Ec2ShopTestRunner.getenv("EC2_SHOP_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = Ec2ShopTestRunner.getenv(key)
        ...
        explain = Ec2ShopTestRunner.getenv("EC2_SHOP_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["EC2_SHOP_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if Ec2ShopTestRunner._test_control is not None:
            return Ec2ShopTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                Ec2ShopTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            Ec2ShopTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = Ec2ShopTestRunner.load_test_control()
...
def load_env_local():
    Ec2ShopTestRunner.load_env_local()


def env_override(m):
    return Ec2ShopTestRunner.env_override(m)


def entity_data(v):
    return Ec2ShopTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return Ec2ShopTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return Ec2ShopTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return Ec2ShopTestRunner.load_test_control()


def live_client_options():
    return Ec2ShopTestRunner.live_client_options()


def live_delay_ms():
    return Ec2ShopTestRunner.live_delay_ms()
