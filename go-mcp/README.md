...
[MCP](https://modelcontextprotocol.io) server exposing the Ec2Shop SDK as
two agent tools — `ec2-shop_list` and `ec2-shop_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ec2-shop-mcp)
make build

# 2. Provide credentials via the environment
export EC2_SHOP_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user ec2-shop \
  -- /absolute/path/to/ec2-shop-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./ec2-shop-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// ec2-shop_list: first page of records
{ "entity": "get_instance_pricing" }
{ "entity": "get_instance_pricing", "query": { } }

// ec2-shop_load: one record by id
{ "entity": "get_instance_pricing", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ec2-shop-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user ec2-shop \
     -- "$PWD"/dist/*/ec2-shop-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `ec2-shop_list` and `ec2-shop_load` tools now appear
   in new sessions. Ask the agent to *"list get_instance_pricing using ec2-shop"*
   and it calls `ec2-shop_list` with `{"entity":"get_instance_pricing"}`.
...
```sh
export EC2_SHOP_APIKEY=sk_live_xxx            # API key
export EC2_SHOP_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `ec2-shop_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `ec2-shop_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `EC2_SHOP_APIKEY` | API key sent with every request. |
| `EC2_SHOP_BASE` | Optional override of the API base URL. |
...
```sh
./ec2-shop-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ec2-shop_load","arguments":{"entity":"get_instance_pricing","query":{"id":1}}}}'
```
