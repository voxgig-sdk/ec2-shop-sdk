...
MCP server exposing the Ec2Shop SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `ec2-shop_list` | `entity`, optional `query` map | First page of records as JSON |
| `ec2-shop_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `ec2-shop_list` and
`ec2-shop_load` tools become available in new sessions.
