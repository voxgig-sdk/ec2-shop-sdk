...
module Ec2ShopUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = Ec2ShopResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = Ec2ShopResponse.new(fetched)
    else
