...
class Ec2ShopRetryFeature extends Ec2ShopBaseFeature
{
    ...
    public function init(Ec2ShopContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (Ec2ShopContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(Ec2ShopContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(Ec2ShopContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
