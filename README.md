...
A simple and fast API to get Amazon EC2 instance pricing right from your terminal.

Learn more about EC2 Shop API at [ec2.shop](https://ec2.shop).
...
This SDK exposes the API as a small set of **semantic entities** — GetInstancePricing — that you
...
const items = await client.GetInstancePricing().list()
```
...
$client = Ec2ShopSDK::test([
    "entity" => ["getinstancepricing" => ["test01" => []]],
]);
$getinstancepricings = $client->GetInstancePricing()->list();
```
...
```go
client := sdk.Test()
result, err := client.GetInstancePricing(nil).List(
    nil, nil,
)
```
...
| --- | --- | --- |
| **GetInstancePricing** | The GetInstancePricing entity (list). | `/` |
...
$getinstancepricings = $client->GetInstancePricing()->list();
print_r(array_map(fn($item) => $item->data_get(), $getinstancepricings));
```
