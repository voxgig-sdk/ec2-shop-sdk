...
class FtCtrl extends Ec2ShopControl
{
...
class FtClient
{
    ...
    private Ec2ShopUtility $utility;

    public function __construct(Ec2ShopUtility $utility, array $options)
    {
    ...
    public function get_utility(): Ec2ShopUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (Ec2ShopContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (Ec2ShopContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = Ec2ShopConfig::make_feature($name);
    ...
    private static function buildUrl(Ec2ShopSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof Ec2ShopError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new Ec2ShopResponse($fetched);
        }
    ...
    private function fail(Ec2ShopContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (Ec2ShopContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
