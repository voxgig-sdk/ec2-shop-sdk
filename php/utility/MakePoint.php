...
// Ec2Shop SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class Ec2ShopMakePoint
{
    public static function call(Ec2ShopContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof Ec2ShopError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = Ec2ShopHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = Ec2ShopHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
