...
class Ec2ShopSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = Ec2ShopHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = Ec2ShopHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!Ec2ShopFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, Ec2ShopFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return Ec2ShopUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = Ec2ShopHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = Ec2ShopHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = Ec2ShopHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new Ec2ShopError($op . "_allow",
                "Ec2ShopSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = Ec2ShopHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new Ec2ShopError("graphql_error",
                "Ec2ShopSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function GetInstancePricing($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_get_instance_pricing === null) {
                $this->_get_instance_pricing = new GetInstancePricingEntity($this, null);
            }
            return $this->_get_instance_pricing;
        }
        return new GetInstancePricingEntity($this, $data);
    }
