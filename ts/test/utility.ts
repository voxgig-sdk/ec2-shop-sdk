...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.EC2SHOP_TEST_LIVE ||
    'TRUE' === process.env.EC2SHOP_TEST_OVERRIDE
  ) {
  ...
  m.EC2SHOP_TEST_EXPLAIN = process.env.EC2SHOP_TEST_EXPLAIN || m.EC2SHOP_TEST_EXPLAIN
