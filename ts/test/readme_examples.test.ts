...
//     (\`new Ec2ShopSDK(...)\` / \`Ec2ShopSDK.test(...)\` become
//     \`Ec2ShopSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"get_instance_pricing":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new Ec2ShopSDK(...)\` and any
// \`Ec2ShopSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+Ec2ShopSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/Ec2ShopSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
