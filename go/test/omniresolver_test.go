...
import (
	...
	omni "github.com/voxgig-sdk/ec2-shop-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.Ec2ShopError); is {
				out := map[string]any{
					"name":    "Ec2ShopError",
...
func omniCtx(arg any, client *sdk.Ec2ShopSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.Ec2ShopSDK); is {
			client = live
