...
func TestGetInstancePricingEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.GetInstancePricing(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.GetInstancePricing(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		getInstancePricingRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.get_instance_pricing")))
...
func get_instance_pricingBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("EC2_SHOP_TEST_GET_INSTANCE_PRICING_ENTID")
	...
	idmapResolved := core.ToMapAny(env["EC2_SHOP_TEST_GET_INSTANCE_PRICING_ENTID"])
