...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("EC2_SHOP_TEST_LIVE") == "TRUE" ||
		os.Getenv("EC2_SHOP_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("EC2_SHOP_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.Ec2ShopSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.Ec2ShopError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.Ec2ShopSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.Ec2ShopError{Msg: msg, Code: code}
}
