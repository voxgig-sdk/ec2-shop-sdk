...
module Ec2ShopOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if Ec2ShopOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = Ec2ShopOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| Ec2ShopOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || Ec2ShopOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = Ec2ShopSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = Ec2ShopError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { Ec2ShopOmni.errify(err) }
