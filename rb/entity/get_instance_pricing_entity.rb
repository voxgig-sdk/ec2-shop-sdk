...
class GetInstancePricingEntity
  ...
  def make
    opts = @_entopts.dup
    GetInstancePricingEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = Ec2ShopHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = Ec2ShopHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [GetInstancePricingListMatch, Hash, nil] match filter (any subset of
  #   GetInstancePricing fields); defaults to nil, treated as an empty match that lists all.
