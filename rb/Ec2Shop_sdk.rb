...
require_relative 'Ec2Shop_types'
...
class Ec2ShopSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = Ec2ShopHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = Ec2ShopHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, Ec2ShopFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = Ec2ShopHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = Ec2ShopHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = Ec2ShopHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = Ec2ShopHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = Ec2ShopError.new(
        "graphql_error", "Ec2ShopSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
