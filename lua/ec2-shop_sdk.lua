...
Ec2ShopSDK._make_feature = _make_feature


function Ec2ShopSDK.new(options)
  local self = setmetatable({}, Ec2ShopSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function Ec2ShopSDK:options_map()
...
function Ec2ShopSDK:get_utility()
  return Utility.copy(self._utility)
end


function Ec2ShopSDK:get_root_ctx()
  return self._rootctx
end


function Ec2ShopSDK:prepare(fetchargs)
...
function Ec2ShopSDK:direct(fetchargs)
...
-- by passing it.
function Ec2ShopSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function Ec2ShopSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "Ec2ShopSDK: graphql: " .. msg
    res.graphql = errors
  end
...
-- Idiomatic facade: client:GetInstancePricing():list() / client:GetInstancePricing():load({ id = ... })
...
function Ec2ShopSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = Ec2ShopSDK.new(sdkopts)
  sdk.mode = "test"
