-- Ec2Shop SDK GetInstancePricing entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local GetInstancePricingEntity = {}
GetInstancePricingEntity.__index = GetInstancePricingEntity


function GetInstancePricingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GetInstancePricingEntity)
  self._name = "get_instance_pricing"
  self._client = client
...
function GetInstancePricingEntity:get_name()
  return self._name
end


function GetInstancePricingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GetInstancePricingEntity.new(self._client, opts)
end
...
function GetInstancePricingEntity:mark_deleted()
  self._deleted = true
end


function GetInstancePricingEntity:deleted()
  return true == self._deleted
end


function GetInstancePricingEntity:data_set(args)
  if args ~= nil then
...
function GetInstancePricingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GetInstancePricingEntity:match_set(args)
  if args ~= nil then
...
function GetInstancePricingEntity:match_get()
...
function GetInstancePricingEntity:stream(action, args, callopts)
...
---@param reqmatch GetInstancePricingListMatch
---@param ctrl? table
---@return GetInstancePricing[]
---@return string? err
function GetInstancePricingEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function GetInstancePricingEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return GetInstancePricingEntity
