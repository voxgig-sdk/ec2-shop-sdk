...
boru-driven command-line client **and** interactive REPL for the Ec2Shop
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ec2-shop-cli)
make build
...
./ec2-shop-cli list get_instance_pricing

# 5. Override the API base URL for a single call
EC2_SHOP_BASE=https://api.example.com ./ec2-shop-cli list get_instance_pricing

# 6. No arguments -> interactive REPL
./ec2-shop-cli
ec2-shop> list get_instance_pricing
ec2-shop> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ec2-shop-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/ec2-shop-cli list get_instance_pricing
   ```
...
Run with no arguments to open a REPL (prompt `ec2-shop>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_instance_pricing`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/ec2-shop-cli`. |
...
(`./ec2-shop-cli <expr>`) and interactively (the REPL), and expressions compose the
